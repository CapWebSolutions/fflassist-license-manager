<?php
/**
 * Class to import FFL License data from a CSV file via WP-CLI command.
 *
 * @package     FFL_Assist
 * @subpackage  Includes
 * @since       1.0.0
 * @category    Wp-ClI_Command
 * @author      Matt Ryan <matt@capwebsolutions.com>
 * @license     GPLv2 (or later) from the [Free Software Foundation]](https://www.fsf.org/). 
...
class Import_Ffl_Data
{
     ...
     * 
     * This class defines a WP-CLI command to import Federal Firearms License (FFL) data from a CSV file. 
     * The CSV file is expected to have specific columns representing different segments of the FFL license number, 
     * along with the licensee's name and business name. The command creates a custom database table to store the imported data.
     * The CSV records structure is as follows. varchar is a variable length string with a maximum length indicated in parentheses: 
     *      ffl_license_number VARCHAR(20)
     *      ffl_license_name VARCHAR(48)
     *      ffl_business_name VARCHAR(48)
     *      ffl_lic_seg1 VARCHAR(1)
     *      ffl_lic_seg2 VARCHAR(2)
     *      ffl_lic_seg3 VARCHAR(3)
     *      ffl_lic_seg4 VARCHAR(2)
     *      ffl_lic_seg5 VARCHAR(2)
     *      ffl_lic_seg6 VARCHAR(5)
     * 
     * ## OPTIONS
     * 
     * --file=<file.csv> 
     ...
     * ## EXAMPLES
     * 
     *    # Import FFL data from a CSV file from the current disk location with logging and a limit of 5000 records.
     *    $ wp import_ffl_data --file=0924-ffl-list-complete.csv --log --limit=5000 
     *    Success: Completed processing 5000 records.
     * 
     *    # Import all of the FFL data records from a CSV file found in the media library.
     *    $ wp import_ffl_data --media http://stg-fflassist-staging.kinsta.cloud/wp-content/uploads/2025/11/110525-ffl-list-30recs.csv
     *    Success: Completed processing 28 records.
     *    
     * ## NOTES
     * 
     * This is a destructive operation as it drops any existing ffl_licensees table before creating a new one.
     * Ensure you have a backup of any existing data before running this command.
     * 
     * @when after_wp_load
     */
