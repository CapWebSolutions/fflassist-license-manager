 ...
 * 
 * --limit=<number>
 *   : If present, the number of records to process.
 * 
 * ## EXAMPLES
 * 
...
class Import_Ffl_Data {
     ...
     public function __invoke($args, $assoc_args) {
        ...
        if ($file_contents === false) {
            WP_CLI::error("Failed to open the file '{$file_path}' using WP_Filesystem.");
            exit;
        }
        ...
        rewind($stream);                    // Return to beginning of memeory file
        if (!$stream) {                     // Opened memory stream okay?
            WP_CLI::error("Failed to open the memory file '{$file_path}'.");
            exit;
        }
